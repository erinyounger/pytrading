...
class BackTestResult(Base):
    ...
    volume_avg_7d = Column(DECIMAL(16, 2), comment='7日平均成交量（万手）')
    atr = Column(DECIMAL(10, 4), comment='平均真实波幅（用于止损计算）')
    is_blacklist = Column(Boolean, default=False, comment='是否在黑名单中')
    industry = Column(String(50), comment='所属行业')
    market_cap = Column(DECIMAL(20, 2), comment='市值（亿元）')
